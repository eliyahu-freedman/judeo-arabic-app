export const meta = {
  name: 'bahya-coverage',
  description: 'Author Bahya Advanced-reader dictionary entries to 100% tap-to-define coverage',
  phases: [
...
let total = (args && args.total) || 2941      // residual group count for round 1

const MERGE_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  required: ['bab1_pct', 'hakdamah_pct', 'residual_groups', 'residual_surfaces', 'applied_entries', 'note'],
  properties: {
    bab1_pct: { type: 'number', description: 'bahya-bab1 covered_pct after this round' },
    hakdamah_pct: { type: 'number', description: 'bahya-hakdamah covered_pct after this round' },
...
If a form is clearly EMBEDDED BIBLICAL/LITURGICAL HEBREW (not Arabic — Bahya quotes Scripture and piyyut heavily), grep the source for context and identify the verse:
  grep -o "...<surface>..." data/bahya-bab1.json data/bahya-hakdamah.json   (or python json load + search)
...
3. Read the final numbers and RETURN them via the StructuredOutput tool:
   - bab1_pct, hakdamah_pct: from data/coverage-advanced.json -> by_text["bahya-bab1"|"bahya-hakdamah"].covered_pct
...
for (let round = 1; round <= MAX_ROUNDS; round++) {
  ...
  log(`Round ${round} done: bab1=${m.bab1_pct}% hakdamah=${m.hakdamah_pct}% | +${m.applied_entries} entries | residual ${m.residual_groups} groups (${m.residual_surfaces} surfaces). ${m.note}`)

  if (m.residual_groups <= 0) { log('🎉 Both Bahya texts at 100%.'); return m }
