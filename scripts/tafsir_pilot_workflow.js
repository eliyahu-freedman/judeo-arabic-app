export const meta = {
  name: 'tafsir-bamidbar-pilot',
  description: 'Pilot: author Tafsir (Bamidbar) dictionary entries toward ~100% coverage',
  phases: [
    { title: 'Author', detail: 'fan out: each agent authors a slice of the Bamidbar residual worklist' },
    { title: 'Merge', detail: 'one agent: concat → apply → re-measure coverage (coverage_report.py)' },
  ],
}

const CHUNK = (args && args.chunk) || 51
...
function mergePrompt() {
  return `MERGE + MEASURE step for the Tafsir Bamidbar pilot. Run from \`/Users/eliyahufreedman/Code/judeo-arabic-app\`.
...
if (m) {
  log(`CHUNK RESULT: +${m.applied_entries} entries | overall hand ${m.overall_hand_pct}% (was 95.66%) | ${m.remaining_groups} Pentateuch groups left. ${m.note}`)
}
return m
